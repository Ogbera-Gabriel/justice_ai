...
const tokenDistribution = [
  { name: 'Liquidity & Ecosystem', shortname: 'L&E', value: 84, color: '#8b5cf6' },
  { name: 'Team', shortname: 'Team', value: 4, color: '#f59e0b' },
  { name: 'Marketing', shortname: 'MKT', value: 5, color: '#ec4899' },
  { name: 'Partnership IBC Group', shortname: 'IBC', value: 3, color: '#3b82f6' },
  { name: 'Charity and Community Rewards', shortname: 'Charity', value: 4, color: '#10b981' },
];
...
export default function TokenChart() {
  ...
  useEffect(() => {
    const updateRadius = () => {
      ...
      if (width < 640) {
        setOuterRadius(80); // For small screens
      } else if (width < 1024) {
        setOuterRadius(120); // For tablets
      } else {
        setOuterRadius(150); // Default for desktops
      }
    };
          ...
          <Card className="bg-gray-900/60 border-purple-500/50 backdrop-blur-sm">
            ...
            <CardContent>
              <div className="flex flex-col lg:flex-row items-center justify-center">
                <ChartContainer
                  config={{}}
                  className="w-full max-w-lg h-[300px] sm:h-[400px] lg:h-[500px]"
                >
